...
export async function fetchAllClientData(client: Client): Promise<Post[]> {
  ...
  const fbFields =
    "id,message,created_time,full_picture,permalink_url,type,attachments{subattachments,media}";
  const fbPublishedPromise = fetchApi(
    `${pageId}/published_posts?access_token=${access_token}&fields=${fbFields}`
  );
  ...
  let igPromise = Promise.resolve(null);
  let igStoriesPromise = Promise.resolve(null);

  if (igAccountId) {
    const igFields =
      "id,caption,media_type,media_url,permalink,timestamp,thumbnail_url,children{media_url,thumbnail_url}";
    igPromise = fetchApi(
      `${igAccountId}/media?access_token=${access_token}&fields=${igFields}`
    );
